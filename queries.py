...
def Q_1(conn, execution_time):
    ...
    query = """
        ...
        JOIN Shots s ON e.event_id = s.event_id
        JOIN Matches m ON e.match_id = m.match_id
        JOIN CompetitionSeasons cs ON m.competition_id = cs.competition_id AND m.season_id = cs.season_id
        WHERE cs.name = 'La Liga' AND cs.season_name = '2020/2021'
...
def Q_2(conn, execution_time):
    ...
    query = """ 
        SELECT
            Players.player_name,
            COUNT(Events.event_id) AS shots_count
        FROM
            Events
        JOIN
            Players ON Events.player_id = Players.player_id
        JOIN
            Matches ON Events.match_id = Matches.match_id
        JOIN
            CompetitionSeasons ON Matches.season_id = CompetitionSeasons.season_id
            AND Matches.competition_id = CompetitionSeasons.competition_id
        WHERE
            CompetitionSeasons.season_name = '2020/2021'
            AND CompetitionSeasons.name = 'La Liga' 
            AND Events.type_id = (SELECT type_id FROM EventTypes WHERE name = 'Shot')
        GROUP BY
            Players.player_name
        HAVING
            COUNT(Events.event_id) >= 1
        ORDER BY
            shots_count DESC,
...
def Q_3(conn, execution_time):
    ...
    query = """ 
        SELECT
            Players.player_name,
            COUNT(Shots.event_id) AS first_time_shots_count
        FROM
            Players
        JOIN
            Events ON Players.player_id = Events.player_id
        JOIN
        ...
        JOIN
            ...
            AND Matches.competition_id = CompetitionSeasons.competition_id
        WHERE
            CompetitionSeasons.season_name IN ('2020/2021', '2019/2020', '2018/2019')
            AND CompetitionSeasons.name = 'La Liga'
            AND Shots.first_time = TRUE
        GROUP BY
            Players.player_name
        HAVING
...
def Q_4(conn, execution_time):
    ...
    query = """ 
        SELECT
            Teams.team_name,
            COUNT(Events.event_id) AS total_passes
        FROM
            Teams
        JOIN
            Events ON Teams.team_id = Events.team_id
        JOIN
            Matches ON Events.match_id = Matches.match_id
        JOIN
            CompetitionSeasons ON Matches.season_id = CompetitionSeasons.season_id
        ...
        WHERE
            CompetitionSeasons.season_name = '2020/2021'
            AND CompetitionSeasons.name = 'La Liga'
            AND Events.type_id = (SELECT type_id FROM EventTypes WHERE name = 'Pass')
        GROUP BY
            Teams.team_name
        HAVING
            COUNT(Events.event_id) >= 1
...
def Q_5(conn, execution_time):
    ...
    query = """ 
        SELECT
            Players.player_name,
            COUNT(Passes.recipient_id) AS intended_recipients
        FROM
            Passes
        JOIN
            Events ON Passes.event_id = Events.event_id
        JOIN
            Players ON Passes.recipient_id = Players.player_id
        JOIN
            Matches ON Events.match_id = Matches.match_id
        JOIN
            CompetitionSeasons ON Matches.season_id = CompetitionSeasons.season_id
            AND Matches.competition_id = CompetitionSeasons.competition_id
        WHERE
            CompetitionSeasons.season_name = '2003/2004'
            AND CompetitionSeasons.name = 'Premier League'
        GROUP BY
            Players.player_name
        HAVING
            COUNT(Passes.recipient_id) >= 1
        ORDER BY
            intended_recipients DESC;
    """
...
def Q_6(conn, execution_time):
    ...
    query = """ 
        SELECT
            Teams.team_name,
            COUNT(Events.event_id) AS total_shots
        FROM
            Teams
        JOIN
            Events ON Teams.team_id = Events.team_id
...
def Q_7(conn, execution_time):
    ...
    query = """
        SELECT
            Players.player_name,
            COUNT(Passes.event_id) AS through_balls_count
        FROM
            Passes
        JOIN
            Events ON Passes.event_id = Events.event_id
        ...
        WHERE
            CompetitionSeasons.season_name = '2020/2021'
            AND CompetitionSeasons.name = 'La Liga'
            AND Passes.through_ball = TRUE 
        GROUP BY
            Players.player_name
        HAVING
            COUNT(Passes.event_id) >= 1
        ORDER BY
            through_balls_count DESC;
    """
...
def Q_8(conn, execution_time):
    ...
    query = """
        ...
        JOIN
            Events ON Passes.event_id = Events.event_id
        JOIN
            Teams ON Events.team_id = Teams.team_id
        JOIN
            Matches ON Events.match_id = Matches.match_id
        JOIN
            CompetitionSeasons ON Matches.season_id = CompetitionSeasons.season_id
...
def Q_9(conn, execution_time):
    ...
    query = """
        ...
        FROM
            Dribbles
        JOIN
            Events ON Dribbles.event_id = Events.event_id
        JOIN
            Players ON Events.player_id = Players.player_id
        JOIN
            Matches ON Events.match_id = Matches.match_id
...
def Q_10(conn, execution_time):
    ...
    query = """
        SELECT
            Players.player_name,
            COUNT(Events.event_id) AS times_dribbled_past
        FROM
            Events
        JOIN
            Players ON Events.player_id = Players.player_id
        ...
        WHERE
            CompetitionSeasons.season_name = '2020/2021'
            AND CompetitionSeasons.name = 'La Liga'
            AND Events.type_id = (SELECT type_id FROM EventTypes WHERE name = 'Dribbled Past')
        GROUP BY
            Players.player_name
        HAVING
            COUNT(Events.event_id) >= 1
